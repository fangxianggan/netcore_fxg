...
namespace NetCore.Core.Helper
{
    ...
    public static class LambdaToSqlHelper<T> where T:class
    {
        ...
        /// <returns></returns>
        public static string GetWhereByLambda(Expression<Func<T, bool>> predicate, DBType dBType)
        {
            bool withQuotationMarks = GetWithQuotationMarks(dBType);

            ConditionBuilder conditionBuilder = new ConditionBuilder();
            conditionBuilder.SetIfWithQuotationMarks(withQuotationMarks); //字段是否加引号（PostGreSql,Oracle）
            conditionBuilder.SetDataBaseType(dBType);
        ...
        /// <returns></returns>
        public static bool GetWithQuotationMarks(DBType dBType)
        {
            bool result = false;
            switch (dBType)
            {
